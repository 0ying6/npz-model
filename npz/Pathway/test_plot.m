%% carbon cycle sysytem
clear,clc

tic
%% initail parameters
epsilon=0.1;
a=0.2;
    b=0.2;
    c=0.4;
    d=1.395;
    e=0.03;
    k=0.05;
    r=0.15;
    s=0.04;
    N_0=0.6;
    alpha=0.25;
    beta=0.33;
    gamma=0.5;
    lamda=0.6;
    mu=0.035;
%% functions 
f = @(x,y,z)    -(x/(e+x))*(a/(b+c*y))*y+r*y+(beta*lamda*(y^2)/(mu^2+y^2))*z+gamma*d*(z^2)+k*(N_0-x);
g = @(x,y,z)    (x/(e+x))*(a/(b+c*y))*y-r*y-(lamda*(y^2)/(mu^2+y^2))*z-(s+k)*y;
h = @(x,y,z)    (alpha*lamda*(y^2)/(mu^2+y^2))*z-d*(z^2);
 
fx = @(x,y,z) (-e/((e+x)^2))*(a/(b+c*y))*y-k;
fxx = @(x,y,z) 2*e*(a*y/(b+c*y))/((e+x)^3);
fxy = @(x,y,z) (-e/((e+x)^2))*a*b/((b+c*y)^2);
fy = @(x,y,z) -(x/(e+x))*a*b/((b+c*y)^2)+r+2*beta*lamda*y*z*(mu^2)/((mu^2+y^2)^2);
fz = @(x,y,z)  beta*lamda*(y^2)/(mu^2+y^2)+gamma*2*d*z;
fxz = @(x,y,z) 0;

gx = @(x,y,z) (a*y/(b+c*y))*e/((e+x)^2);
gy = @(x,y,z) x/(e+x)*a*b/((b+c*y)^2)-r-2*lamda*y*z*mu^2/((mu^2+y^2)^2)-s-k;
gz = @(x,y,z)  -lamda*y^2/(mu^2+y^2);
gyx = @(x,y,z)  (a*b/(b+c*y)^2)*e/((e+x)^2);
gyy = @(x,y,z)  -2*a*b*c*x/(e+x)/((b+c*y)^3)-2*lamda*z*(mu^4-3*mu^2*y^2)/((mu^2+y^2)^3);
gyz = @(x,y,z)   -2*lamda*mu^2*y/((mu^2+y^2)^2);

hx = @(x,y,z)  0;
hy = @(x,y,z)  2*lamda*alpha*z*y*mu^2/((mu^2+y^2)^2);
hz = @(x,y,z)  alpha*lamda*(y^2)/(mu^2+y^2)-2*d*z;
hzx = @(x,y,z)  0;
hzy = @(x,y,z)  2*lamda*alpha*y*mu^2/((mu^2+y^2)^2);
hzz = @(x,y,z)  -2*d;
%% stable point
u_L =0.2261;
v_L = 0.0468;
w_L = 0.0690;
%% final point in limit cycle
AA = load ('LimitCycle_11.txt');
BB = load ('LimitCycle_11_ans.csv');

%% test point 
[NN1 NN2] = size(AA);
a1 = AA(1,14500);
a2 = AA(2,14500);
a3 = AA(3,14500);
%% submit neural shooting result
v1 = BB(1,14500);
v2 = BB(2,14500);
v3 = BB(3,14500);
%% main code
T = 4;
% tspan = [0,T]; 
% opts=odeset('RelTol',1e-2,'AbsTol',1e-4);
% y0 = [u_L v1 v_L v2];
% dy = @(t, y)[y(2); -(y(2)-f(y(1),y(3)))^2*sx(y(1),y(3))/s(y(1),y(3))+y(2)*2*sx(y(1),y(3))*(y(2)-f(y(1),y(3)))/s(y(1),y(3))+f(y(1),y(3))*fx(y(1),y(3))+s(y(1),y(3))^2*fxx(y(1),y(3))/2+(g(y(1),y(3))*gx(y(1),y(3))*s(y(1),y(3))^2)/ga(y(1),y(3))^2+gxy(y(1),y(3))*s(y(1),y(3))^2/2+y(4)*(fy(y(1),y(3))-s(y(1),y(3))^2*gx(y(1),y(3))/ga(y(1),y(3))^2); y(4); y(2)*(gx(y(1),y(3))-fy(y(1),y(3))*ga(y(1),y(3))^2/s(y(1),y(3))^2)+ f(y(1),y(3))*fy(y(1),y(3))*ga(y(1),y(3))^2/s(y(1),y(3))^2+fyx(y(1),y(3))*ga(y(1),y(3))^2/2+g(y(1),y(3))*gy(y(1),y(3))+gyy(y(1),y(3))*ga(y(1),y(3))^2/2];
% 
% % 
% [x,u2] = ode45(dy,tspan,y0,opts);
% y= FUNC(dy,0,0.01,4,y0);

%% Euler method
N = 10^4;
dt = T/N;
t = 0 : dt : T;
F = @(x,y,z,u,v,w) x^(-1)*y^2+fy(x,z,v)*u+fz(x,z,v)*w-x^(-1)*f(x,z,v)^2+f(x,z,v)*fx(x,z,v)-x^2/z^2*(u-g(x,z,v))*gx(x,z,v)-x^2/v^2*(w-h(x,z,v))*hx(x,z,v)+((epsilon^2*x^2)/2)*(fxx(x,z,v)+gyx(x,z,v)+hzx(x,z,v));
G = @(x,y,z,u,v,w) z^(-1)*u^2+gx(x,z,v)*y+gz(x,z,v)*w-z^(-1)*g(x,z,v)^2+g(x,z,v)*gy(x,z,v)-z^2/x^2*(y-f(x,z,v))*fy(x,z,v)-z^2/v^2*(w-h(x,z,v))*hy(x,z,v)+((epsilon^2*z^2)/2)*(fxy(x,z,v)+gyy(x,z,v)+hzy(x,z,v));
H = @(x,y,z,u,v,w) v^(-1)*w^2+hx(x,z,v)*y+hy(x,z,v)*u-v^(-1)*h(x,z,v)^2+h(x,z,v)*hz(x,z,v)-v^2/x^2*(y-f(x,z,v))*fz(x,z,v)-v^2/z^2*(u-g(x,z,v))*gz(x,z,v)+((epsilon^2*v^2)/2)*(fxz(x,z,v)+gyz(x,z,v)+hzz(x,z,v));

x1(1) = u_L;
x2(1) = v1;
x3(1) = v_L;
x4(1) = v2;
x5(1) = w_L;
x6(1) = v3;
for i = 1 : N
    x1(i+1) = x1(i) + x2(i)*dt;
    x2(i+1) = x2(i) + F(x1(i),x2(i),x3(i),x4(i),x5(i),x6(i))*dt;
    x3(i+1) = x3(i) + x4(i)*dt;
    x4(i+1) = x4(i) + G(x1(i),x2(i),x3(i),x4(i),x5(i),x6(i))*dt;
    x5(i+1) = x5(i) + x6(i)*dt;
    x6(i+1) = x6(i) + H(x1(i),x2(i),x3(i),x4(i),x5(i),x6(i))*dt;
end

plot3(x1,x3,x5,'r'); hold on%鐢荤殑鏄建閬?plot(x1(end),x3(end),'o')
plot3(AA(1,:),AA(2,:),AA(3,:),'b'); hold on%鐢绘瀬闄愮幆 
plot3(a1,a2,a3,'*');hold on%鏋侀檺鐜笂鐨勭洰鏍囩偣



%鍐嶇敾涓?釜
a1 = AA(1,12600);
a2 = AA(2,12600);
a3 = AA(3,12600);
%% submit neural shooting result
v1 = BB(1,12600);
v2 = BB(2,12600);
v3 = BB(3,12600);
%% main code
T = 4;
% tspan = [0,T]; 
% opts=odeset('RelTol',1e-2,'AbsTol',1e-4);
% y0 = [u_L v1 v_L v2];
% dy = @(t, y)[y(2); -(y(2)-f(y(1),y(3)))^2*sx(y(1),y(3))/s(y(1),y(3))+y(2)*2*sx(y(1),y(3))*(y(2)-f(y(1),y(3)))/s(y(1),y(3))+f(y(1),y(3))*fx(y(1),y(3))+s(y(1),y(3))^2*fxx(y(1),y(3))/2+(g(y(1),y(3))*gx(y(1),y(3))*s(y(1),y(3))^2)/ga(y(1),y(3))^2+gxy(y(1),y(3))*s(y(1),y(3))^2/2+y(4)*(fy(y(1),y(3))-s(y(1),y(3))^2*gx(y(1),y(3))/ga(y(1),y(3))^2); y(4); y(2)*(gx(y(1),y(3))-fy(y(1),y(3))*ga(y(1),y(3))^2/s(y(1),y(3))^2)+ f(y(1),y(3))*fy(y(1),y(3))*ga(y(1),y(3))^2/s(y(1),y(3))^2+fyx(y(1),y(3))*ga(y(1),y(3))^2/2+g(y(1),y(3))*gy(y(1),y(3))+gyy(y(1),y(3))*ga(y(1),y(3))^2/2];
% 
% % 
% [x,u2] = ode45(dy,tspan,y0,opts);
% y= FUNC(dy,0,0.01,4,y0);

%% Euler method
N = 10^4;
dt = T/N;
t = 0 : dt : T;
F = @(x,y,z,u,v,w) x^(-1)*y^2+fy(x,z,v)*u+fz(x,z,v)*w-x^(-1)*f(x,z,v)^2+f(x,z,v)*fx(x,z,v)-x^2/z^2*(u-g(x,z,v))*gx(x,z,v)-x^2/v^2*(w-h(x,z,v))*hx(x,z,v)+((epsilon^2*x^2)/2)*(fxx(x,z,v)+gyx(x,z,v)+hzx(x,z,v));
G = @(x,y,z,u,v,w) z^(-1)*u^2+gx(x,z,v)*y+gz(x,z,v)*w-z^(-1)*g(x,z,v)^2+g(x,z,v)*gy(x,z,v)-z^2/x^2*(y-f(x,z,v))*fy(x,z,v)-z^2/v^2*(w-h(x,z,v))*hy(x,z,v)+((epsilon^2*z^2)/2)*(fxy(x,z,v)+gyy(x,z,v)+hzy(x,z,v));
H = @(x,y,z,u,v,w) v^(-1)*w^2+hx(x,z,v)*y+hy(x,z,v)*u-v^(-1)*h(x,z,v)^2+h(x,z,v)*hz(x,z,v)-v^2/x^2*(y-f(x,z,v))*fz(x,z,v)-v^2/z^2*(u-g(x,z,v))*gz(x,z,v)+((epsilon^2*v^2)/2)*(fxz(x,z,v)+gyz(x,z,v)+hzz(x,z,v));

    x1(1) = u_L;
    x2(1) = v1;
    x3(1) = v_L;
    x4(1) = v2;
    x5(1) = w_L;
    x6(1) = v3;
for i = 1 : N
    x1(i+1) = x1(i) + x2(i)*dt;
        x2(i+1) = x2(i) + F(x1(i),x2(i),x3(i),x4(i),x5(1),x6(i))*dt;
        x3(i+1) = x3(i) + x4(i)*dt;
        x4(i+1) = x4(i) + G(x1(i),x2(i),x3(i),x4(i),x5(i),x6(i))*dt;
        x5(i+1) = x5(i) + x6(i)*dt;
        x6(i+1) = x6(i) + H(x1(i),x2(i),x3(i),x4(i),x5(i),x6(i))*dt;
end

plot3(x1,x3,x5,'r'); hold on%鐢荤殑鏄建閬?plot(x1(end),x3(end),'o')
plot3(a1,a2,a3,'*');hold on


toc